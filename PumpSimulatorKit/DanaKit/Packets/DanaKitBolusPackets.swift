...
extension DanaKitMessages {
    ...
    private static func scheduleBolusTime(amount: UInt16, speed: BolusSpeed, _ model: DanaKitProcessMessage) {
        ...
        model.state.bolusTotal = Double(amount) / 100
        ...
        DispatchQueue.main.async {
            bolusTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in
                ...
                if progress >= 1 {
                    ...
                    model.state.bolusTotal = nil
                    model.state.bolusProgress = nil
                    model.pumpManager.notifyStateUpdate()
                    ...
                    let currentAmountProgress = UInt16(progress * Double(amount))
                    let currentAmount = roundToSupportedBolusVolume(Double(currentAmountProgress) / 100)

                    model.state.reservoirLevel = originalReservoirLevel - currentAmount
                    model.state.bolusProgress = currentAmount
                    model.pumpManager.notifyStateUpdate()

                    let message = DanaKitEncryption.encrypt(
                        data: Data([
                            UInt8(currentAmountProgress & 0xFF),
                            UInt8(currentAmountProgress >> 8),
                        ]),
                        type: DanaKitMessageType.TYPE_NOTIFY,
